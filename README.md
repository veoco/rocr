...
- `--threads <n>`：CPU 推理线程数（默认 1）。candle 默认按物理核数建线程池，
  对 PP-OCRv6 这类卷积密集小算子在多核机器上反而慢一个数量级；单线程是最
  保守的默认，最优线程数因硬件而异，可试 `--threads 4`~`8` 并实测；
...
以下为本机（CPU，release 构建，单线程）对一张 900×700 文档图的整页 OCR 耗时。
rocr 默认将 candle 的线程池限制为单线程（见 `--threads`）——candle 默认按物理
核数建池，在多核机器上反而因 barrier 同步开销比单线程慢一个数量级（本机 59 s →
6.8 s）。最优线程数因硬件而异，可试 `--threads 4`~`8`。
